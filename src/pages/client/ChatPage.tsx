...
function ChatPage() {
  ...
  const userType = localStorage.getItem('client');

    const fetchData = async () => {
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/${userType}/getAllMessengers`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem(userType?'clientToken': 'freelancerToken')}`
        }
      })
    ...
    useEffect(()=>{
      const recieveMessage = async () =>{
        const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/getAllMessages`,{
          from: currentUserId,
          to: activeMessenger
        })
        ...
        </Box>
        {
          allUsers.map((user: any) => {
            return  <Box onClick={() => {
              ...
              fetchData()
            }} sx={{width: '280px', height: '70px', backgroundColor: `${activeMessenger===user._id ? '#002D04':'white'}`, borderRadius: '5px'}} className='clientActiveMessenger'>
              <Stack direction='row'>
              <Avatar
